  ...
  describe '一覧表示機能' do
    ...
    context 'タスクが作成日時の降順に並んでいる場合' do
      it '新しいタスクが一番上に表示される' do
        ...
        expect(task_list[0]).to have_content "最初に作成しあタスク"
        expect(task_list[1]).to have_content "２番目に作成したタスク"
        expect(task_list[2]).to have_content "３番目に作成したタスク"
      end
    end
    context 'タスクが終了期限の降順に並んでいる場合' do
      it '終了期限のもっと近いタスクが一番上に表示される' do
        task_list = all('.task_row')
        click_on '終了期限でソートする'
        expect(task_list[0]).to have_content "2020-01-01"
        expect(task_list[1]).to have_content "2019-01-01"
        expect(task_list[2]).to have_content "2018-01-01"
      end
    end
  end
  describe '詳細表示機能' do
